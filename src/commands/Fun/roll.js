    ...
    try {
      ...
      if (numDice < 1 || numDice > 20) {
        throw new TitanBotError(
          ...
          'Veuillez choisir un nombre de dés compris entre 1 et 20.'
        );
      }
      ...
      if (numSides < 1 || numSides > 1000) {
        throw new TitanBotError(
          ...
          ErrorTypes.VALIDATION,
          'Veuillez choisir un nombre de faces entre 1 et 1000.'
        );
      }
      ...
      const resultsDetail =
        numDice > 1 ? `**Dés :** ${rolls.join(" + ")}\n` : "";
      const modifierText = modifier !== 0 ? ` + (${modifier})` : "";

      const embed = successEmbed(
        `🎲 Lancement : ${numDice}d${numSides}${modifier !== 0 ? match[3] : ""}`,
