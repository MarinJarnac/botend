            ...
            subcommand
                ...
                .addChannelOption((option) =>
                    option
                        .setName("category")
                        .setDescription(
                            "La catégorie dans laquelle les nouveaux tickets seront créés (optionnel).",
                        )
                ...
                .addRoleOption((option) =>
                    option
                        .setName("staff_role")
                        .setDescription(
                            "Le rôle qui aura accès aux tickets (optionnel).",
                        )
                        .setRequired(false),
                )
                .addIntegerOption((option) =>
                    option
                        ...
                        .setDescription("Nombre maximum de tickets qu'un utilisateur peut ouvrir (par défaut : 3)")
                        .setMinValue(1)
                        .setMaxValue(10)
                        .setRequired(false),
                )
                .addBooleanOption((option) =>
                    option
                        .setName("dm_on_close")
                        .setDescription("Envoyer un message privé à l'utilisateur quand son ticket est fermé (par défaut : true)")
                        .setRequired(false),
                ),
        )
        .addSubcommand((subcommand) =>
    ...
    async execute(interaction, config, client) {
        try {
            ...
            if (
                ...
                return await InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        errorEmbed(
                            "Permission refusée",
                            "Vous devez disposer de la permission `Gérer les salons` pour effectuer cette action.",
                        ),
                    ],
                });
            }
            ...
            if (subcommand === "setup") {
                    ...
                    return await InteractionHelper.safeEditReply(interaction, {
                        embeds: [
                            errorEmbed(
                                ...
                                `Ce serveur possède déjà un système de tickets configuré (panneau dans <#${existingConfig.ticketPanelChannelId}>).\n\nUn seul système de tickets est supporté par serveur. Utilisez \`/ticket dashboard\` pour modifier ou mettre à jour la configuration actuelle, ou sélectionnez **Supprimer le système** depuis le tableau de bord pour tout réinitialiser.`,
                            ),
                        ],
                    });
                }
                ...
                const panelMessage = interaction.options.getString("panel_message") || "Cliquez sur le bouton ci-dessous pour ouvrir un ticket d'assistance.";
                const buttonLabel = interaction.options.getString("button_label") || "Créer un ticket";
                ...
                try {
                    ...
                    if (closedCategoryChannel) {
                        successMessage += `Les tickets fermés seront déplacés dans **${closedCategoryChannel.name}**. `;
                    }
                    ...
                    successMessage += `\n\n**Tickets max par utilisateur :** ${maxTicketsPerUser}\n**Notification MP à la fermeture :** ${dmOnClose ? 'Activée' : 'Désactivée'}`;
                    ...
                    const logEmbed = createEmbed({
                        title: "🔧 Configuration du système de tickets (Log)",
                    ...
                    .addFields(
                        ...
                        {
                            name: "Notification MP",
                            value: dmOnClose ? 'Activée' : 'Désactivée',
                            inline: true,
                        },
                        {
                            name: "Modérateur",
                        ...
                        await InteractionHelper.safeEditReply(interaction, {
                            embeds: [
                                errorEmbed(
                                    ...
                                    "Impossible d'envoyer le panneau de tickets ou de sauvegarder la configuration. Vérifiez les permissions du bot (notamment sa capacité à envoyer des messages dans le salon cible) ainsi que la connexion à la base de données.",
                                ),
                            ],
                        }).catch(err => {
